...
# ✅ Import `sandboxItems` from `ItemsSandbox.py`
try:
    import mjolnir.ItemsSandbox  # Import the module
    reload(mjolnir.ItemsSandbox)  # Ensure the latest version is loaded
    from mjolnir.ItemsSandbox import sandboxItems  # Load the dictionary
except ModuleNotFoundError as e:
    print(f"Error: Could not import ItemsSandbox.py - {e}")
    sandboxItems = {}  # Fallback to an empty dictionary if loading fails
...
def getItemName(index):
    return sandboxItems.get(index, 'Unknown')  # Get the item name or return 'Unknown' if not found
